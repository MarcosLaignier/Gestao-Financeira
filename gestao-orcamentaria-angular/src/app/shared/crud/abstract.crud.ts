...
export abstract class AbstractCrud<T,F> extends BaseComponent{
  ...
  /** Metodo chamado antes da execução do Delete,
   * Pode ser usado para alterar algum dado do model antes de invocar o Delete*/

  beforeDoDelete(): Observable<any> | null {
    return null;
  }

  /** Metodo chamado depois do beforeDoDelete e antes da execução do Delete */
  ...
  /** Metodo chamado para o delete */
  doDelete(model:any) {
    this.beforeDoDelete();
    if(this.validateDelete()){
      this.getMainService().delete(model.id).subscribe(res => {
        if (res.status == HttpStatusCode.NoContent) {
          notify('Excluido com sucesso', 'success', 3000);
          window.history.back();
        }
        return res;
      },error => {
        notify('Algo deu errado ao tentar excluir', 'error', 3000);
        return error;
